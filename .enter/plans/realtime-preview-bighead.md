...
支付方式需要支持 Stripe 或支付宝（Alipay）。

## 架构设计 (Architecture)
我们将使用 **Stripe** 作为支付网关，因为 Stripe 原生支持开启 Alipay（支付宝）和 WeChat Pay（微信支付）作为支付选项。

整体业务链路如下：
1. **点击下载**：用户点击“下载”按钮。
2. **创建订单**：前端调用 Enter Cloud (Supabase) 的 Edge Function (`create-checkout-session`)。
3. **跳转支付**：获取到 Stripe Checkout URL 后，将用户重定向到 Stripe 托管的支付页面（在此页面用户可以选择信用卡或支付宝）。
